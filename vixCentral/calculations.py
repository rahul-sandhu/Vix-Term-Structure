...
# VIX
vix = pd.read_json(
    f"https://fmpcloud.io/api/v3/historical-price-full/%5EVIX?from={from_date}&to={to_date}&apikey={API_KEY}")
vix = vix['historical']
df['F0'] = [vix[i]['close'] for i in range(len(vix))]

# SPY
spy = pd.read_json(
    f"https://fmpcloud.io/api/v3/historical-price-full/SPY?from={from_date}&to={to_date}&apikey={API_KEY}")
spy = spy['historical']
